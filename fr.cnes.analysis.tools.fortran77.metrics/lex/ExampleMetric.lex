...
/*******************************************************************************/
/* This file is an example on how to edit a metric with JFlex, in order to fit */
/* with plug-in's definitions. Important point that must not be changed are    */
/* described. Moreover, implementation part that only depend on the Metric are   */
...
/* Variable "location" is used to determine Metric's error location (function,	*/
/* procedure, etc.).															*/
...
%{
	...
	private void setValue(String location, int line){
		fileValue.getFunctionValues().add( new FunctionMetricValue(location, locationValue ) );
	}
%}

/* At the end of analysis, the fileValue is thrown */
%eofval{
...
<YYINITIAL>		{METRIC_WORD}		{this.setError(location, yyline + 1);
								 yybegin(LINE);}
...
/* This what has to be done whenever an error is found. */
<NEW_LINE>		{METRIC_WORD}		{this.setError(location, yyline + 1);
								 yybegin(LINE);}
...
/* This what has to be done whenever an error is found. */
<LINE> 			{METRIC_WORD}		{this.setError(location, yyline + 1);}
