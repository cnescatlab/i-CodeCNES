...
!************************************************************************
!
! But: Passage du repere equatorial moyen J2000 au repere equatorial moyen a la date t
! ===
!
...
!$Historique
! ==========
!   + Version 2.0 (SP 381 ed01 rev00): creation a partir de la routine MRChgRepJ2000CM de la MSLIB f77
...
real(pm_reel), dimension(3) :: vit_EquaMoy_iner, vit_entrainement ! vecteur vitesse dans le repere equatorial vrai 
...
character(len=pm_longueur_info_utilisateur), parameter :: info_utilisateur = &
     '@(#) Fichier MSLIB mr_J2000_EquaMoy.f90: derniere modification V6.13 >'

! Ne pas toucher a la ligne suivante
character(len=pm_longueur_rcs_id), parameter :: rcs_id =' $Id: mr_J2000_EquaMoy.f90 362 2013-02-15 18:01:28Z bbjc $ '
...
call mu_quat_rep (pos_J2000, quat, pos_EquaMoy, code_retour_local)
if (code_retour_local%valeur /= pm_OK) then
   code_retour%valeur = code_retour_local%valeur
   if (code_retour_local%valeur < pm_OK) go to 6000
end if
...
if  ((present(vit_J2000)) .and. (present(vit_EquaMoy))) then

   call mu_quat_rep (vit_J2000, quat, vit_EquaMoy_iner, code_retour_local)
   if (code_retour_local%valeur /= pm_OK) then
      code_retour%valeur = code_retour_local%valeur
      if (code_retour_local%valeur < pm_OK) go to 6000
   end if
...
if (present(jacob)) then
   ...
   if  ((present(vit_J2000)) .and. (present(vit_EquaMoy))) then ! partie liee aux vitesses
...
code_retour%routine = pm_num_mr_J2000_EquaMoy
