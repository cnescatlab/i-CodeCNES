...
public class TestGlobal_1 {
	...
	/**
	 * Function test to validate global test. This function do
	 *  - get all files in /resources folder
	 *  - run analysis in this files
	 *  - export results to globalTest.txt file
	 *  - compare file with the last one
	 *  
	...
	public void runGlobalTest() {
		try {
			...
			final RuleAnalysisJob analysis = new RuleAnalysisJob(
					ruleExtensionId, listFiles);
	...
	private void getFilesIntoFolder(File folder) {
		if(folder.isDirectory()) {
			...
			for(File file : listOfFiles) {
				if(file.isDirectory()) getFilesIntoFolder(file);
				else {
	...
	/**
	 * Function to create the error file. The file has the following:
	 *  RuleName FileName NumError
	 */
	 ...
	 * 
	 * @return
	 * 		equals indicate the equality of the files
	 */
