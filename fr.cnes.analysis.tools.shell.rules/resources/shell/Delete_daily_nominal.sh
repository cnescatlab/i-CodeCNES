#!/bin/sh
############################################################################################
#Script : Delete_daily_nominal.sh
...
# Date : 26/06/2014
# Versio  :V1 command rm -rf *$param* => rm -rf $param* to avoid deletion of wrong session  
# Version :V0
# Date : 24/03/2014
############################################################################################
FILETYPE=DailyNominal
PRAMETERS=$@
display_usage() {
        echo -e "\n*********************************************************"
        echo -e "**                      << USAGE >>                    **"
...
#                        ERRORFILE=/GFS_GBIN/tests/testTar/tBackup/Backup/log/${FILETYPE}_$param.err      #SET Log only when an error occure
			...
			LOGDELETE=/GFS_SAGA/DPCCTools/TarTools/log/DailyNominal/${FILETYPE}_$param.del
                        cd /GFS_BACKUP/Aspera/OPS/reception/daily/nominal                               #SET Folder containing architecture needed to be Tar
                        LOGFILE=/GFS_SAGA/DPCCTools/TarTools/log/DailyNominal/${FILETYPE}_$param.log    #SET log destination
                        ERRORFILE=/GFS_SAGA/DPCCTools/TarTools/log/DailyNominal/${FILETYPE}_$param.err   #SET Log only when an error occure
                        ###########################################################
		 if [ -f $LOGDELETE ]; then

                        echo "RUN DATE    -- `date +%Y-%m-%d_%H-%M-%S`" | tee -a $LOGDELETE             #date to Log
                        echo "SCRIPT INFO -- $0 Parameters : $PARAMETERS" | tee -a $LOGDELETE
                        echo -e "\n********************* Working Directories  *****************\n" | tee -a $LOGDELETE
                        echo "'cd'/GFS_BACKUP/Aspera/OPS/reception/daily/nominal" | tee -a $LOGDELETE
                        echo "'LOGFILE:'/GFS_SAGA/DPCCTools/TarTools/log/DailyNominal/${FILETYPE}_$param.log" | tee -a $LOGDELETE
                        echo "'ERRORFILE:'/GFS_SAGA/DPCCTools/TarTools/log/DailyNominal/${FILETYPE}_$param.err" | tee -a $LOGDELETE
			...
			echo "RUN DATE    -- `date +%Y-%m-%d_%H-%M-%S`" | tee $LOGDELETE        											     #date to Log
                        echo "SCRIPT INFO -- $0 Parameters : $PARAMETERS" | tee -a $LOGDELETE
                        echo -e "\n********************* Working Directories  *****************\n" | tee -a $LOGDELETE
                        echo "'cd'/GFS_BACKUP/Aspera/OPS/reception/daily/nominal" | tee -a $LOGDELETE
                        echo "'LOGFILE:'/GFS_SAGA/DPCCTools/TarTools/log/DailyNominal/${FILETYPE}_$param.log" | tee -a $LOGDELETE
                        echo "'ERRORFILE:'/GFS_SAGA/DPCCTools/TarTools/log/DailyNominal/${FILETYPE}_$param.err" | tee -a $LOGDELETE                        
                        chmod 750 $LOGDELETE    				                                       							 #give right to group group_group to check log 
                        echo -e "\n********************* Deletion test  *****************\n" | tee -a $LOGDELETE
                                        ...
                                        if [ -f ${FILETYPE}_${param: -4}.tar ]; then
                                                ...
                                                echo "WARN--${FILETYPE}_${param}.tar file found, removal might already been applied. Action: Skipped and nothing deleted" | tee -a $LOGDELETE
                                                        ...
                                                        if [ $nbTar -eq 0 ];then
                                                                ...
                                                                echo "Number of gbin file in current architecture before deletion : $nbGbin" | tee -a $LOGDELETE
                                                                nbTarGbin=$(tar tvf ${FILETYPE}_${param: -4}.tar | grep gbin |  wc -l) 
                                                                echo "Number of gbin file in the tar : $nbTarGbin" | tee -a $LOGDELETE

                                                                nbStartStop=$(ls daily_*_$param*.xml | wc -l)
                                                                echo "Number of start and stop files in current architecture before deletion : $nbStartStop" | tee -a $LOGDELETE
                                                                nbTarStartStop=$(tar tvf ${FILETYPE}_${param: -4}.tar | grep daily |  wc -l)
                                                                echo "Number of start and stop file in the tar : $nbTarStartStop" | tee -a $LOGDELETE

                                                                        if test $nbTarGbin -eq $nbGbin && test $nbStartStop -eq $nbTarStartStop; then
                                                                                rm -rf $param* | tee -a $LOGDELETE
                                                                                echo "[OK] : CLEAR--Action : File within $param* removed " | tee -a $LOGDELETE
										rm -f daily_*_$param*.xml | tee -a $LOGDELETE
                                                                                echo "[OK] : CLEAR--Action : File within daily_*_$param*.xml removed " | tee -a $LOGDELETE
                                                                                mv ${FILETYPE}_${param: -4}.tar ${FILETYPE}_${param}.tar | tee -a $LOGDELETE
                                                                                echo "[OK] : CLEAR--Action : file ${FILETYPE}_${param: -4}.tar renamed ${FILETYPE}_${param}.tar" | tee -a $LOGDELETE
                                                                        else
                                                                        echo "WARN--difference of Gbin file number beetween tar file and architecture for day : *$param*. Action: Skipped and nothing deleted" | tee -a $LOGDELETE
                                                                        fi
                                                                else
