#!/bin/bash
echo "------------------------------------------"
echo "COM.PRES.LENGTHLINE"i
echo "Fichier OK  de TU"
echo "------------------------------------------"

code_erreur_nbargs=-128

# Dénition de la variable qui contiendra le code retour (en particulier p
# pour les cas d'erreur avec valeurs negatives)
...
affiche_resultat ()
{
   ...
   then
      ...
      if [ $p2 -ge 0 ]
      then
         printf "execution la fonction  avec chaine de calcul: %s resultat = %s \n" $p1 $2
         else
         printf "erreur d'execution de avec chaine de calcul : %s code retour = %s\n" $p1 $p2
