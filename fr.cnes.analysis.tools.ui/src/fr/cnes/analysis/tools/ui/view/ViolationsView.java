	...
	public void update() {
		for (final ViewerFilter filter : this.viewer.getFilters()) {
			if (filter instanceof IUpdatableAnalysisFilter) {
				((IUpdatableAnalysisFilter) filter).update(searchString, showInfo, showWarning, showError);
			}
		}
		viewer.refresh();
	}
		...
		synchronized (this) {
			final Set<CheckResult> listInputs = new TreeSet<CheckResult>(new Comparator<CheckResult>() {
				...
				public int compare(final CheckResult check1, final CheckResult check2) {
					int res = check1.getName().split("\\.")[0].compareTo(check2.getName().split("\\.")[0]);
					if (res == 0) {
						res = check1.getName().split("\\.")[1].compareTo(check2.getName().split("\\.")[1]);
						if (res == 0) {
							res = check1.getName().split("\\.")[2].compareTo(check2.getName().split("\\.")[2]);
							if (res == 0) {
								res = check1.getFile().getAbsolutePath().compareTo(check2.getFile().getAbsolutePath());
								if (res == 0) {
									res = check1.getLine().compareTo(check2.getLine());
									if (res == 0) {
										res = check1.getLocation().compareTo(check2.getLocation());
									}
								}
							}
						}
