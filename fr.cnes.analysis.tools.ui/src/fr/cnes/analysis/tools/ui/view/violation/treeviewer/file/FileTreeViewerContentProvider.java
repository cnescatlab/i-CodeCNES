...
public class FileTreeViewerContentProvider implements ITreeContentProvider {
	/** Logger **/
	private static final Logger LOGGER = Logger.getLogger(FileTreeViewerContentProvider.class.getName());
		...
		try {
			if (newInput instanceof CheckResult[]) {
				...
				final UnknownInstanceException exception = new UnknownInstanceException(
						"inputChanged method of AbstractContentProvider has a " + newInput.getClass().getName()
	...
	public Object[] getChildren(final Object parentElement) {
		...
		if (parentElement instanceof FileRuleDescriptor) {
			...
			final List<FunctionDescriptor> mVals = ((FileRuleDescriptor) parentElement).getDescriptors();
			...
			final List<RuleDescriptor> mVals = ((FunctionDescriptor) parentElement).getDescriptors();
			values = mVals.toArray(new RuleDescriptor[mVals.size()]);
		} else if (parentElement instanceof RuleDescriptor) {
			final List<ViolationDescriptor> mVals = ((RuleDescriptor) parentElement).getDescriptors();
