...
public class ViolationToFileTreeViewerConverter extends Job {
	/** Logger. **/
	public final static Logger LOGGER = Logger.getLogger(ViolationToFileTreeViewerConverter.class.getName());
		...
		try {
			...
			for (final CheckResult value : this.inputs) {
				...
				if (descriptors.contains(file)) {
					...
					function = new FunctionDescriptor(value.getLocation(), -1,
							new Path(value.getFile().getAbsolutePath()));
					if (file.getDescriptors().contains(function)) {
						function = file.getDescriptors().get(file.getDescriptors().indexOf(function));
						rule = new RuleDescriptor(value.getId(), value.getName(), value.getLocation(), -1,
								new Path(value.getFile().getAbsolutePath()));
						if (function.getDescriptors().contains(rule)) {
							rule = function.getDescriptors().get(function.getDescriptors().indexOf(rule));
							viold = new ViolationDescriptor(value.getName(), value.getLocation(), value.getMessage(),
									value.getLine(), new Path(value.getFile().getAbsolutePath()));
			...
			status = new Status(Status.ERROR, "fr.cnes.analysis.tools.ui", Status.ERROR, exception.getMessage(),
					exception);
		}
