 ...
 * 
 */
public class ViolationToRuleTreeViewerConverter extends Job {
    /** Logger. **/
    public final static Logger LOGGER = Logger.getLogger(ViolationToRuleTreeViewerConverter.class.getName());
     ...
     */
    public ViolationToRuleTreeViewerConverter() {
     ...
     *            the inputs
     */
    public ViolationToRuleTreeViewerConverter(final Violation[] pInputs) {
        super("Converting results...");
        ...
        try {
            for (final Violation value : this.inputs) {
                if (descriptors.isEmpty() || !descriptors.get(descriptors.size() - 1).getName()
                        .equals(value.getRuleName())) {
                    rule.getDescriptors().clear();
                    rule.setRuleId(value.getRuleId());
                    rule.setName(value.getRuleName());
