...
!************************************************************************
!
! But:  Passage du repere celeste intermediaire CIP au repere terrestre vrai
...
! Autres declarations
! ===================
real(pm_reel), dimension(3,3) :: R,RT                                        ! matrice de changement de repere
real(pm_reel), dimension(3,3) :: M_rot, M_rot_RT                             ! matrices intermédiaires
real(pm_reel), dimension(3)   :: vit_TerVrai_inertiel,vect_rot2,vect_entrainement ! vecteur intermediaire
integer                       :: ii, jj
...
character(len=pm_longueur_info_utilisateur), parameter :: info_utilisateur = &
     '@(#) Fichier MSLIB mr_CIP_TerVrai_iers2003.f90: derniere modification V6.13.1 >'

! Ne pas toucher a la ligne suivante
character(len=pm_longueur_rcs_id), parameter :: rcs_id =' $Id: mr_CIP_TerVrai_iers2003.f90 362 2013-02-15 18:01:28Z bbjc $ '
...
! - Calcul de theta(t_tu1_j2000) selon la methode de l'IERS
...
! matrice de changement de repere RT=transposé de R
call mu_transpose3(R,RT,code_retour_local)
if (code_retour_local%valeur /= pm_OK) then
    code_retour%valeur = pm_err_calc_mat
    go to 6000
end if

! - Calcul de pos_TerVrai=R*pos_CIP
call mu_mulvect3(RT,pos_CIP,pos_TerVrai, code_retour_local)
if (code_retour_local%valeur /= pm_OK) then
   code_retour%valeur = pm_err_calc_transfo
   go to 6000
end if

! - Calcul de vit_TerVrai
if (present(vit_TerVrai)) then
  if (inertiel2) then ! calcul inertiel
  
    ! vit_TerVrai=RT*vit_CIP
    call mu_mulvect3(RT,vit_CIP,vit_TerVrai, code_retour_local)
    if (code_retour_local%valeur /= pm_OK) then
      code_retour%valeur = pm_err_calc_transfo
      go to 6000
    end if
  ...
  ! vecteur intermediaire : vit_TerVrai_inertiel = RT*vit_CIP
  call mu_mulvect3(RT,vit_CIP,vit_TerVrai_inertiel, code_retour_local)
  if (code_retour_local%valeur /= pm_OK) then
    code_retour%valeur = pm_err_calc_transfo
    go to 6000
  end if
  ! vecteur intermedaire de rotation : vect_rot = [0 0 -theta_point]t
  vect_rot2(1)=0._pm_reel
  vect_rot2(2)=0._pm_reel
  vect_rot2(3)=-pm_deux_pi*(1._pm_reel+pm_DM1092_TerVrai_CIP_cte2)/nb_sec_par_jour
  ! vecteur intemediaire d'entrainement : vect_entrainement = vect_rot .prodvect. pos_TerVrai 
  call mu_prod_vect (vect_rot2, pos_TerVrai, vect_entrainement, code_retour)
  if (code_retour_local%valeur /= pm_OK) then
    code_retour%valeur = pm_err_calc_transfo
    go to 6000 
  end if
  ! vit_TerVrai = vit_TerVrai_inertiel+vect_entrainement
  vit_TerVrai(1)=vit_TerVrai_inertiel(1)+vect_entrainement(1)
  vit_TerVrai(2)=vit_TerVrai_inertiel(2)+vect_entrainement(2)
  vit_TerVrai(3)=vit_TerVrai_inertiel(3)+vect_entrainement(3)
  end if
...
if (present(jacob)) then
  if (inertiel2) then ! calcul inertiel
    ...
    call mu_matmul3(M_rot,RT,M_rot_RT,code_retour_local)
    if (code_retour_local%valeur /= pm_OK) then
      code_retour%valeur = pm_err_calc_mat
      go to 6000
    end if
...
code_retour%routine = pm_num_mr_CIP_TerVrai_iers
