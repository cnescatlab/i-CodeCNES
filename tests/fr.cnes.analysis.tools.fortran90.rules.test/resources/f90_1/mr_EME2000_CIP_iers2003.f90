...
!************************************************************************
!
! But:  Passage du repere equatorial moyen EME2000 ou repere celeste intermediaire CIP
...
real(pm_reel), dimension(3,3) :: QT, Q_intermediaire, R, M_rot_1, M_rot_2    ! matrices intermédiaires
real(pm_reel), dimension(3)   :: omega, vit_CIP_inertiel,vit_CIP_entrainement ! vecteurs intermediaires
integer                       :: ii, jj
...
character(len=pm_longueur_info_utilisateur), parameter :: info_utilisateur = &
     '@(#) Fichier MSLIB mr_EME2000_CIP_iers2003.f90: derniere modification V6.13 >'

! Ne pas toucher a la ligne suivante
character(len=pm_longueur_rcs_id), parameter :: rcs_id =' $Id: mr_EME2000_CIP_iers2003.f90 362 2013-02-15 18:01:28Z bbjc $ '
...
! matrice Q_transpose=Q_intermediaire*R
...
! matrice QT
call mu_transpose3(Q,QT,code_retour_local)
if (code_retour_local%valeur /= pm_OK) then
    code_retour%valeur = pm_err_calc_mat
    go to 6000
end if

! - Calcul de pos_CIP=Q*pos_EME2000

call mu_mulvect3(QT,pos_EME2000,pos_CIP, code_retour_local)
if (code_retour_local%valeur /= pm_OK) then
...
if (present(vit_CIP)) then
   ...
   if (inertiel2) then ! calcul inertiel
      ...
      ! vit_CIP=Q*vit_EME2000
      call mu_mulvect3(QT,vit_EME2000,vit_CIP, code_retour_local)
      if (code_retour_local%valeur /= pm_OK) then
         code_retour%valeur = pm_err_calc_transfo
         go to 6000
      end if
      ...
      call mu_mulvect3(QT,vit_EME2000,vit_CIP_inertiel, code_retour_local)
      if (code_retour_local%valeur /= pm_OK) then
         code_retour%valeur = pm_err_calc_transfo
         go to 6000
      end if

      ! vitesse intemediaire d'entrainement : vit_CIP_entrainement = omega .prodvect. pos_CIP
      call mu_prod_vect (omega, pos_CIP, vit_CIP_entrainement, code_retour)
      if (code_retour_local%valeur /= pm_OK) then
         code_retour%valeur = pm_err_calc_transfo
         go to 6000 
      end if
      
      ! vit_CIP=vit_CIP_inertiel+vit_CIP_entrainement
      vit_CIP(1)=vit_CIP_inertiel(1)+vit_CIP_entrainement(1)
      vit_CIP(2)=vit_CIP_inertiel(2)+vit_CIP_entrainement(2)
      vit_CIP(3)=vit_CIP_inertiel(3)+vit_CIP_entrainement(3)
...
if (present(jacob)) then
  ...
  do ii=1,3
   ...
   ! matrice intermediaire M_rot_2=QT*M_rot_1
   call mu_matmul3(QT,M_rot_1,M_rot_2,code_retour_local)
   if (code_retour_local%valeur /= pm_OK) then
     code_retour%valeur = pm_err_calc_mat
     go to 6000
   end if
...
code_retour%routine = pm_num_mr_EME2000_CIP_iers
