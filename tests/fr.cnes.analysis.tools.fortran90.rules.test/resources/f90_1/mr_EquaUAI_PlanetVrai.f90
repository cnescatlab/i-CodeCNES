...
!************************************************************************
!
! But:  Passage du repere equatorial planetaire UAI au repere planetocentrique
! ===
!
! Note d'utilisation:  
! ==================
...
!$Historique
! ==========
!   + Version 5.0 (SP 497 ed01 rev00): creation
...
!       valeur_code_retour_mslib, numero_routine_mslib et longueur_chaine_mslib
...
character(len=pm_longueur_info_utilisateur), parameter :: info_utilisateur = &
                     '@(#) Fichier MSLIB mr_EquaUAI_PlanetVrai.f90: derniere modification V6.13 >'

! Ne pas toucher a la ligne suivante
character(len=pm_longueur_rcs_id), parameter :: rcs_id =' $Id: mr_EquaUAI_PlanetVrai.f90 362 2013-02-15 18:01:28Z bbjc $ '
...
call mu_quat_rep(pos_EquaUAI,quat,pos,code_retour_local)
...
if  (present(vit_EquaUAI).and.present(vit_PlanetVrai)) then      ! calcul des composantes vitesses

   call mu_quat_rep(vit_EquaUAI,quat,vit,code_retour_local)
   if (code_retour_local%valeur /= pm_OK)  then
      code_retour%valeur = code_retour_local%valeur
      if (code_retour_local%valeur < pm_OK)  go to 6000
   end if
...
code_retour%routine = pm_num_mr_EquaUAI_PlanetVrai
