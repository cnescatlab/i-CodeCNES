...
!************************************************************************
!
! But: Passage du repere equatorial vrai a la date t au repere terrestre vrai a la meme date t 
! ===
!
...
!$Historique
! ==========
!   + Version 4.0 (SP 453 ed01 rev00): creation a partir de mr_EquaVrai_veis
...
!   + Version 6.0 (DE 564 ed01 rev00): ajout de l'option inertiel (calcul sans vitesse d'entrainement) 
...
real(pm_reel), dimension(3)   :: vit_TerVrai_iner, vit_entrainement ! vecteur vitesse dans le repere terrestre vrai 
...
character(len=pm_longueur_info_utilisateur), parameter :: info_utilisateur = &
     '@(#) Fichier MSLIB mr_EquaVrai_TerVrai.f90: derniere modification V6.13 >'

! Ne pas toucher a la ligne suivante
character(len=pm_longueur_rcs_id), parameter :: rcs_id = &
     ' $Id: mr_EquaVrai_TerVrai.f90 362 2013-02-15 18:01:28Z bbjc $ '
...
call mu_quat_rep (pos_EquaVrai, quat, pos_TerVrai, code_retour_local)
if (code_retour_local%valeur /= pm_OK) then
   code_retour%valeur = code_retour_local%valeur
   if (code_retour_local%valeur < pm_OK) go to 6000
end if
...
if  ((present(vit_EquaVrai)) .and. (present(vit_TerVrai))) then

   call mu_quat_rep (vit_EquaVrai, quat, vit_TerVrai_iner, code_retour_local)
   if (code_retour_local%valeur /= pm_OK) then
      code_retour%valeur = code_retour_local%valeur
      if (code_retour_local%valeur < pm_OK) go to 6000
   end if
   ...
   if (flag_iner) then ! vecteur vitesse cas inertiel
      ...
      call mu_prod_vect(vect_omega, pos_TerVrai, vit_entrainement, code_retour_local)
      if (code_retour_local%valeur /= pm_OK) then
         code_retour%valeur = code_retour_local%valeur
         if (code_retour_local%valeur < pm_OK) go to 6000
      end if
      ...
      vit_TerVrai(:) = vit_TerVrai_iner(:) - vit_entrainement(:)
...
if (present(jacob)) then
   ...
   if  ((present(vit_EquaVrai)) .and. (present(vit_TerVrai))) then ! partie liee aux vitesses
...
code_retour%routine = pm_num_mr_EquaVrai_TerVrai
