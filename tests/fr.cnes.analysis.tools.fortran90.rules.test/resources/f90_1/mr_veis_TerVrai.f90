...
!************************************************************************
!
! But: Passage du repere de Veis a la date t au repere terrestre vrai a la meme date t
! ===
!
...
!$Historique
! ==========
!   + Version 2.0 (SP 377 ed01 rev00): creation a partir de la routine MRChgRepVeisTV de la MSLIB f77
...
real(pm_reel), dimension(3)   :: vit_TerVrai_iner, vit_entrainement ! vecteur vitesse dans le repere de Terrestre Vrai
...
character(len=pm_longueur_info_utilisateur), parameter :: info_utilisateur = &
     '@(#) Fichier MSLIB mr_veis_TerVrai.f90: derniere modification V6.13 >'

! Ne pas toucher a la ligne suivante
character(len=pm_longueur_rcs_id), parameter :: rcs_id =' $Id: mr_veis_TerVrai.f90 362 2013-02-15 18:01:28Z bbjc $ '
...
call mu_quat_rep (pos_veis, quat, pos_TerVrai, code_retour_local)
if (code_retour_local%valeur /= pm_OK) then
   code_retour%valeur = code_retour_local%valeur
   if (code_retour_local%valeur < pm_OK) go to 6000
end if
...
if  ((present(vit_veis)) .and. (present(vit_TerVrai))) then

   call mu_quat_rep (vit_veis, quat, vit_TerVrai_iner, code_retour_local)
   if (code_retour_local%valeur /= pm_OK) then
      code_retour%valeur = code_retour_local%valeur
      if (code_retour_local%valeur < pm_OK) go to 6000
   end if
...
if (present(jacob)) then
   ...
   if  ((present(vit_veis)) .and. (present(vit_TerVrai))) then ! partie liee aux vitesses
...
code_retour%routine = pm_num_mr_veis_TerVrai
