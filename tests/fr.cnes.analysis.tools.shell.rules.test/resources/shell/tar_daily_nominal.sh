#!/bin/sh
############################################################################################
#Script : Launch_daily_nominal_tar.sh
# Build Tar file for Daily nominal folder in GFS_BACKUP to prevent operating system files overflow
...
#                       ERRORFILE=/GFS_GBIN/tests/testTar/tBackup/Backup/log/${FILETYPE}_$param.err      		#SET Log only when an error occure
			...
			echo "SCRIPT INFO -- $0 Parameters -- $PARAMETERS" | tee -a $LOGFILE            #date to Log
                        ...
                        echo "'CD' -- /GFS_BACKUP/Aspera/OPS/reception/daily/nominal" | tee -a $LOGFILE
                        echo "'LOGFILE -- '/GFS_SAGA/DPCCTools/TarTools/log/DailyNominal/${FILETYPE}_$param.log" | tee -a $LOGFILE
			ERRORFILE=/GFS_SAGA/DPCCTools/TarTools/log/DailyNominal/${FILETYPE}_$param.err 			#SET Log only when an error occure
                        echo "'ERRORFILE -- '/GFS_SAGA/DPCCTools/TarTools/log/DailyNominal/${FILETYPE}_$param.err" | tee -a $LOGFILE
                        echo -e "\n***********************************************\n" | tee -a $LOGFILE
			chmod 750 $LOGFILE
