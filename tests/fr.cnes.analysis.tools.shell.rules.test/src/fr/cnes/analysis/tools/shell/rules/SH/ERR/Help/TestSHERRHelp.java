...
public class TestSHERRHelp {
	...
	public final AbstractChecker rule = new SHERRHelp();
	...
	public void testRunWithError2() {
		...
		try {
			// Initializing rule and getting error file.
			final File file = new File(FileLocator.resolve(getClass().getResource(ERROR_FILE_2)).getFile());
			...
			final int nb_CheckResults = list.size();
			assertEquals("Wrong number of CheckResults : ", LINES_2.length, nb_CheckResults);
			...
			for (final CheckResult value : list) {
				...
				final String location = value.getLocation();
				assertTrue("CheckResult " + index.toString() + " has wrong location : " + location + " should contain "
						+ LOCATIONS_2[index], location.contains(LOCATIONS_2[index]));
				final int line = value.getLine();
				assertEquals("CheckResult " + index.toString() + " is in wrong line : ", LINES_2[index], line);
			}
		} catch (final FileNotFoundException e) {
			fail("Erreur d'analyse (FileNotFoundException)");
		} catch (final IOException e) {
