...
package fr.cnes.analysis.tools.shell.rules.SH.ERR.Help;
...
import fr.cnes.analysis.tools.shell.rules.SHERRHelp;
 ...
 * 
 */
public class TestSHERRHelp {

    public final static String   ERROR_FILE    = "error.sh";
    public final static String   ERROR_FILE_2  = "error2.sh";
    public final static String   NO_ERROR_FILE = "noError.sh";
    public final static int[]    LINES         = { 40 };
    public final static int[]    LINES_2       = { 43 };
    public final static String[] LOCATIONS     = { "getopts_internal" };
    public final static String[] LOCATIONS_2   = { "getopts_internal" };
    public final AbstractRule    rule          = new SHERRHelp();
    ...
    public void testRunWithError() {
        ...
        try {
            ...
            final IPath file = new Path(this.getClass().getResource(ERROR_FILE).getPath());
            ...
            for (final Violation value : list) {
                ...
                assertTrue("Violation " + index.toString() + " has wrong location : " + location
                            + " should contain " + LOCATIONS[index],
                            location.contains(LOCATIONS[index]));
                final int line = value.getLine();
                assertEquals("Violation " + index.toString() + " is in wrong line : ", LINES[index],
                            line);
            }
     ...
     */
    @Test
    public void testRunWithError2() {

        try {
            // Initializing rule and getting error file.
            final IPath file = new Path(this.getClass().getResource(ERROR_FILE_2).getPath());
            ...
            final int nb_violations = list.size();
            assertEquals("Wrong number of violations : ", LINES_2.length, nb_violations);

            // We verify that the error detected is the right one. There is
            // only one case of error : a blank common (with no name) is
            // detected.
            final String fileName = list.get(0).getFilePath().toFile().getName();
            assertEquals("Wrong file name : ", ERROR_FILE_2, fileName);
            ...
            for (final Violation value : list) {
                ...
                final String location = value.getLocation();
                assertTrue("Violation " + index.toString() + " has wrong location : " + location
                            + " should contain " + LOCATIONS_2[index],
                            location.contains(LOCATIONS_2[index]));
                final int line = value.getLine();
                assertEquals("Violation " + index.toString() + " is in wrong line : ",
                            LINES_2[index], line);
            }
        } catch (final FileNotFoundException e) {
            fail("Erreur d'analyse (FileNotFoundException)");
    ...
    public void testRunWithoutError() {
        try {
            // Initializing rule and getting error file.
            final IPath file = new Path(this.getClass().getResource(NO_ERROR_FILE).getPath());
